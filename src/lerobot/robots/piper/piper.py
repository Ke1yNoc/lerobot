...
class Piper(Robot):
    ...
    def __init__(self, config: PiperConfig, **kwargs):
        """
        ...
        self.sdk = PiperSDKInterface(port=config.can_interface, gripper_serial = config.gripper_serial)
    ...
    def _motors_ft(self) -> dict[str, type]:
        EEF_KEYS = ["X","Y","Z","RX","RY","RZ","Gripper_dis"]
    ...
    def send_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        a = action.detach().float().cpu().view(-1)
        a_np = a.numpy()
        eef_cmd = a_np[:6].astype(np.float32) 
        positions = [
            a_np[0],
            a_np[1],
            a_np[2],
