...
USB_PORTS["1-2.1:1.0"]="can0:1000000"
USB_PORTS["1-2.2:1.0"]="can1:1000000"
...
for iface in $SYS_INTERFACE; do
    ...
    # 兼容可能出现的前缀，例如“usb-1-2.1:1.0”，只保留端口信息部分
    BUS_INFO=${BUS_INFO#usb-}
    ...
    if [ -n "${USB_PORTS[$BUS_INFO]}" ];then
        ...
        # 使用字符串比较避免 CURRENT_BITRATE 为空时的算术错误
        if [ "$IS_LINK_UP" = "yes" ] && [ "$CURRENT_BITRATE" = "$TARGET_BITRATE" ]; then
            ...
            if [ "$iface" != "$TARGET_NAME" ]; then
                if ip link show "$TARGET_NAME" &>/dev/null; then
                    echo "[WARN]: Skipping rename; target '$TARGET_NAME' already exists."
                    echo "[HINT]: Free the name or adjust USB_PORTS if renaming is required."
                else
            ...
            # 仅当当前名称与目标名称不同且目标名称被其它接口占用时才视为重命名冲突
            if [ "$iface" != "$TARGET_NAME" ] && ip link show "$TARGET_NAME" &>/dev/null; then
                echo "[WARN]: Target name '$TARGET_NAME' already in use by another interface; skipping rename."
                echo "[HINT]: Adjust USB_PORTS mapping or free the name to allow renaming."
                # 不再中止流程，继续设置比特率和激活
            fi
            # if ip link show "$TARGET_NAME" &>/dev/null; then
